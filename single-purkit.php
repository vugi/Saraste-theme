...
<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
				<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
					...
					<?php
						$asdasd = array("Lippukunta", "Vaikeusaste", "Koordinaatit", "Ohjeet");
						
						foreach($asdasd as $field){
							if(get_post_meta($post->ID, $field, true)){
								$meta = get_post_meta(get_the_ID(), $field, true);
								echo '<p class="meta ' . $field . '"><span>' . $field . '</span><span' .($field == "Vaikeusaste" ? ' class="' . $meta. '"' : '') . '>' . $meta . '</span></p>';
							}
						}
					?>
